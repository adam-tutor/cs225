...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    string _word1 = word1;
    string _word2 = word2;
    std::transform(_word1.begin(), _word1.end(), _word1.begin(), ::toupper);
    std::transform(_word2.begin(), _word2.end(), _word2.begin(), ::toupper);

    if(dict.find(_word1) == dict.end() || dict.find(_word2) == dict.end() || dict.at(_word1).size() != dict.at(_word2).size()){
        return false;
    }
    for(unsigned i = 0; i < dict.at(_word1).size(); i++){
        if(dict.at(_word1).at(i) != dict.at(_word2).at(i)){
            return false;
        }
    }
    return true;
