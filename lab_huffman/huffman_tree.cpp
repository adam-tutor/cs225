...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(singleQueue.size() + mergeQueue.size() != 1){
      smallest_1 = removeSmallest(singleQueue, mergeQueue);
      smallest_2 = removeSmallest(singleQueue, mergeQueue);
      TreeNode* both = new TreeNode(smallest_1->freq.getFrequency() + smallest_2->freq.getFrequency());
      both->left = smallest_1;
      both->right = smallest_2;
      mergeQueue.push(both);
    }
    if(singleQueue.empty()){
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
  ...
  if(!bfile.hasBits()){
    ...
    TreeNode* leaf = new TreeNode(new_frequency);
    return leaf;
  }
  else{
    Frequency new_frequency(0);
    next = new TreeNode(new_frequency);
