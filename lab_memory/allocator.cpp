...
void Allocator::loadRooms(const std::string& file)
{
    ...
    //fileio::getNumRooms();
    roomCount = fileio::getNumRooms(); //Added so that we are able to use the variable roomCount in the next line, otherwise roomCount = NULL
    ...
    while (fileio::areMoreRooms()) {
        ...
        i++; //moved i++ from the top of the while loop to the bottom
    }
}
