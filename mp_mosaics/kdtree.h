...
class KDTree
{
  ...
  private:
     ...
     */
    typename KDTree<Dim>::KDTreeNode* copy( KDTreeNode* current_node);
    void destroy(KDTreeNode* current_node);
    vector<Point<Dim>> newPoints_;
    Point<Dim> findNearest(const Point<Dim>& query, int dim, KDTreeNode* curRoot) const;
    typename KDTree<Dim>::KDTreeNode* buildTree(vector<Point<Dim>>& newPoints_, int dim, int left, int right, KDTreeNode* curRoot);
    Point<Dim>& quickSelect(vector<Point<Dim>>& newPoints_, int left, int right, int dim, unsigned int k);
    int partition(vector<Point<Dim>>& newPoints_, int left, int right, int dim, unsigned int pivotIndex);
};
